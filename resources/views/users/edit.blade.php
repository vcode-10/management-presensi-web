                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input id="crud-form-nip" name="nip" type="number" class="form-control w-full"
                                placeholder="NIP" min="0" maxlength="18" value="{{ old('nip', $user->nip) }}">
                            @error('nip')
                        ...
                        <div class="form-group">
                            ...
                            <select id="crud-form-gender" name="gender" class="form-control w-full">
                                <option value="male" @if (old('gender', $user->gender) === 'male') selected @endif>Laki - Laki
                                </option>
                                <option value="female" @if (old('gender', $user->gender) === 'female') selected @endif>Perempuan</option>
                            </select>
                            @error('gender')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select id="crud-form-role" name="role_id" class="form-control w-full">
                                @foreach ($roles as $item)
                                    <option value="{{ $item->id }}" @if (old('role_id', $user->role_id) == $item->id) selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('role_id')
